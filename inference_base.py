...
from litgpt.model_v2 import GPT, Config
...
def next_token_v2(model: GPT, x: torch.Tensor, **kwargs: Any) -> torch.Tensor:
    logits = model(x)
    next = sample(logits[:, -1:], **kwargs)
    return next.to(dtype=x.dtype)
...
def generate(
    ...
    *,
    ...
    for _ in range(1, max_returned_tokens - T + 1):
        token = next_token_v2(model, prompt.view(1, -1), temperature=temperature, top_k=top_k, top_p=top_p).clone()
        prompt = torch.concat([prompt, token], dim=-1)
